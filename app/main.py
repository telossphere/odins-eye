...
app = FastAPI(
    title="Odin's AI",
    description="Advanced AI/ML Platform",
    version="1.0.0"
)
...
async def gpu_monitor(request: Request):
    """GPU monitoring page"""
...
async def gpu_detailed():
    ...
    try:
        ...
        result = subprocess.run([
            'nvidia-smi', 
            '--query-gpu=index,name,memory.total,memory.used,memory.free,temperature.gpu,utilization.gpu,power.draw,power.limit,clocks.current.graphics,clocks.current.memory',
                ...
                if line.strip():
                    ...
                    if len(parts) >= 11:
                        gpu = {
                            ...
                            "memory_percent": round((int(parts[3]) / int(parts[2])) * 100, 1),
...
async def gpu_processes():
    ...
    try:
        ...
        result = subprocess.run([
            'nvidia-smi', 
            '--query-compute-apps=gpu_uuid,pid,process_name,used_memory,process_type',
            '--format=csv,noheader,nounits'
        ], capture_output=True, text=True, timeout=5)
                        ...
                        try:
                            if parts[1] != 'N/A':
                                ps_result = subprocess.run([
                                    'ps', '-p', parts[1], '-o', 'user,pcpu,pmem,etime,command', '--no-headers'
                                ...
                                if ps_result.returncode == 0 and ps_result.stdout.strip():
                                    ...
                                    if len(ps_parts) >= 4:
                                        process_info.update({
                                            "user": ps_parts[0],
                                            "cpu_percent": float(ps_parts[1]) if ps_parts[1] != '-' else 0,
                                            "memory_percent": float(ps_parts[2]) if ps_parts[2] != '-' else 0,
                                            "runtime": ps_parts[3],
                                            "command": ' '.join(ps_parts[4:]) if len(ps_parts) > 4 else ""
                                        })
                        except:
                            pass
